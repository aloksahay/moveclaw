...
**Open Claw Hackathon @ ETH Denver** — Store messages on-chain on Movement Network.
...
- **Functions**: `set_message`, `get_message` — store and retrieve a string per address
...
One-time setup — initialize CLI and fund your testnet account:
...
├── components/
│   ├── message-contract.tsx     # MoveClaw message UI
│   ├── wallet-demo-content.tsx
...
- `.movement/` and `.aptos/` — CLI config (contains private keys)
- `config.yaml`
- `*.key`, `*.secret`
